...
function aprobar(id, eo){
  $.ajax({
    ...
    success: function (response) {
        if(response == 1)
        {
            $('#controles-'+id).html('<p><span class="label label-success">Permitido</span></p><button class="btn btn-default" onclick="aprobar('+id+', 0)">Suspender</button><a href="perfil1/'+id+'" class="btn btn-warning btn-sm">editar</a></p>');
        }
        else if(response == 0)
        {
            $('#controles-'+id).html('<p><span class="label label-danger">Suspendido</span></p><button class="btn btn-info" onclick="aprobar('+id+', 1)">Permitir</button><a href="perfil1/'+id+'" class="btn btn-warning btn-sm">editar</a></p>');
        }
        else {
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/test_2_1/resources/views/admin/dashboard.blade.php ENDPATH**/ ?>